...
export async function ReadyToDispatchPane() {
  ...
  if (topics.length === 0) {
    return (
      <section className="rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-6">
        ...
        <p className="mt-2 text-sm text-text-secondary">
          ...
          <Link href="/" className="text-[var(--accent-electric)] hover:underline">
            Cockpit
          </Link>{" "}
          first.
        </p>
  ...
  return (
    <section className="rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-surface)]">
      <header className="flex items-center justify-between px-4 py-3 border-b border-[var(--border-subtle)]">
        <h2 className="text-lg font-semibold text-text-primary">Ready to Dispatch</h2>
        <span className="text-xs font-mono text-[var(--text-muted)]">{topics.length} reviewed</span>
      </header>
      <ul className="divide-y divide-subtle">
        {topics.map((t) => (
          <li key={t.id} className="flex items-center gap-4 px-4 py-3">
            <span className="font-mono text-lg text-[var(--accent-electric)] w-10 shrink-0">
              {t.hookability_score ?? "—"}
            </span>
